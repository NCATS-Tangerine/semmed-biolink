...
3. Manually unzip the UMLS_ARCHIVE, extract the contained files including the embedded `mmsys.zip` archive. This archive contains the UMLS tool for building components of the US NIH NLM Metathesaurus needed for some necessary SemMedDb postprocessing.  Running the 'mmsys' application on your chosen platform brings up a Metamorphosys application dialog. Select "Install UMLS" and you only need to select "Metathesaurus". When prompted for a configuration, selection "New Configuration" which will ask you to select the level of vocabulary extraction. Choose "Level 0". Modify any additional input and output details in the full dialog, then initiate subset generation (Note: the way to do this seems to subtly vary between operating systems so read the [Metamorphosys documentation](https://www.nlm.nih.gov/research/umls/implementation_resources/metamorphosys/help.html) for clarification). After the vocabulary subsets are generated, apply the following (*nix OS) operations on the indicated "Rich Release Format" ("RRF") files that were generated by the above operation, to prepare them for use in the SemMedDb processing protocols:
