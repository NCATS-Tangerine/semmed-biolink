...
#
# SemMedDb 2020 data release values taken as defaults in April 2021 script
...
echo
echo "SEMMEDDB_DOWNLOAD_PATH:  ${SEMMEDDB_DOWNLOAD_PATH}"
echo "SEMMEDDB_VERSION: ${SEMMEDDB_VERSION}"
echo "SEMMEDDB_PUBMED_RELEASE: ${SEMMEDDB_PUBMED_RELEASE}"

# Data file targets:
SEMMEDDB_FILE_PREFIX=semmedVER${SEMMEDDB_VERSION}
echo "SEMMEDDB_FILE_PREFIX: ${SEMMEDDB_FILE_PREFIX}"

# SemMedDb 2019 data release file name in original Greg Stupp script
# SEMMEDDB_PREDICATION_FILE=${SEMMEDDB_FILE_PREFIX}_R_PREDICATION_${SEMMEDDB_PUBMED_RELEASE}
# Revised file name format in SemMedDb 2020 data release

# Predication Table Data Targets:
: ${SEMMEDDB_PREDICATION_FILE:=${SEMMEDDB_FILE_PREFIX}_${SEMMEDDB_PUBMED_RELEASE}_R_PREDICATION}

export SEMMEDDB_PREDICATION_ARCHIVE=${SEMMEDDB_PREDICATION_FILE}.sql.gz
export SEMMEDDB_PREDICATION_DOWNLOAD=${SEMMEDDB_DOWNLOAD_PATH}${SEMMEDDB_PREDICATION_ARCHIVE}
...
SEMMEDDB_CITATIONS_FILE=${SEMMEDDB_FILE_PREFIX}_${SEMMEDDB_PUBMED_RELEASE}_R_CITATIONS
export SEMMEDDB_CITATIONS_ARCHIVE=${SEMMEDDB_CITATIONS_FILE}.sql.gz
export SEMMEDDB_CITATIONS_DOWNLOAD=${SEMMEDDB_DOWNLOAD_PATH}${SEMMEDDB_CITATIONS_ARCHIVE}
...
SEMMEDDB_PREDICATION_AUX_FILE=${SEMMEDDB_FILE_PREFIX}_${SEMMEDDB_PUBMED_RELEASE}_R_PREDICATION_AUX
export SEMMEDDB_PREDICATION_AUX_ARCHIVE=${SEMMEDDB_PREDICATION_AUX_FILE}.sql.gz
export SEMMEDDB_PREDICATION_AUX_DOWNLOAD=${SEMMEDDB_DOWNLOAD_PATH}${SEMMEDDB_PREDICATION_AUX_ARCHIVE}
...
SEMMEDDB_SENTENCE_FILE=${SEMMEDDB_FILE_PREFIX}_${SEMMEDDB_PUBMED_RELEASE}_R_SENTENCE
export SEMMEDDB_SENTENCE_ARCHIVE=${SEMMEDDB_SENTENCE_FILE}.sql.gz
export SEMMEDDB_SENTENCE_DOWNLOAD=${SEMMEDDB_DOWNLOAD_PATH}${SEMMEDDB_SENTENCE_ARCHIVE}
...
#
UMLS_VERSION=${UMLS_YEAR}${UMLS_TAG}
: ${UMLS_VERSION:="2021AB"}
...
METAMAP_VERSION=${METAMAP_YEAR}${METAMAP_TAG}
